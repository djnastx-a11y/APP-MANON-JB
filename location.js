...
function markerIcon(label, partner = false) {
  return L.divIcon({className:"person-marker",html:`<div class="person-pin ${partner ? "partner" : ""}"><span>${label}</span><i></i></div>`,iconSize:[84,98],iconAnchor:[42,88]});
}
...
function updateMap(rows) {
  ...
  valid.forEach(row => {
    ...
    if (!markers.has(key)) markers.set(key, L.marker(latlng,{icon:markerIcon(label,partner),keyboard:false,riseOnHover:true}).addTo(map));
    else markers.get(key).setLatLng(latlng);
    if (!partner) {
      const accuracy = Number(row.accuracy_m);
      if (accuracyCircle) accuracyCircle.remove();
      if (Number.isFinite(accuracy) && accuracy > 0 && accuracy <= 500) accuracyCircle = L.circle(latlng,{radius:accuracy,className:"accuracy-ring",stroke:true,fill:true,interactive:false}).addTo(map);
    }
  });
  [...markers.keys()].forEach(key => { if (!seen.has(key)) { markers.get(key).remove(); markers.delete(key); } });
  if (!hasAutoFramed && valid.length) { hasAutoFramed = true; setTimeout(() => fitEveryone(false), 120); }
}
...
async function loadPositions({quiet=false} = {}) {
  ...
  const {data,error} = await supabase.from("current_locations").select("user_id,latitude,longitude,accuracy_m,altitude_m,speed_mps,heading_deg,source,captured_at,received_at").order("captured_at",{ascending:false});
  if (error) { if (!quiet) setMessage("Impossible de charger les positions.", true); return; }
...
async function saveSharingPreference(enabled) {
  const {error} = await supabase.from("location_sharing_preferences").upsert({user_id:currentUser.id,sharing_enabled:enabled,precise_enabled:true,updated_at:new Date().toISOString()});
...
async function persistPosition(position, forceHistory = false) {
  ...
  const row = {user_id:currentUser.id,latitude:c.latitude,longitude:c.longitude,accuracy_m:Number.isFinite(c.accuracy)?c.accuracy:null,altitude_m:Number.isFinite(c.altitude)?c.altitude:null,speed_mps:Number.isFinite(c.speed)&&c.speed>=0?c.speed:null,heading_deg:Number.isFinite(c.heading)&&c.heading>=0&&c.heading<360?c.heading:null,source:"web",captured_at:new Date(position.timestamp || Date.now()).toISOString(),received_at:new Date().toISOString()};
...
async function startSharing() {
  ...
  navigator.geolocation.getCurrentPosition(async firstPosition => {
    try {
      await saveSharingPreference(true); setLiveUi(true); await persistPosition(firstPosition,true); focusUser(false);
      if (watchId!==null) navigator.geolocation.clearWatch(watchId);
      watchId = navigator.geolocation.watchPosition(p => persistPosition(p).catch(() => setMessage("La position n’a pas pu être synchronisée.",true)),e => setMessage(geoErrorText(e),true),{enableHighAccuracy:true,maximumAge:3000,timeout:15000});
    } catch (e) { console.error(e); setMessage("Impossible d’activer le partage.",true); setLiveUi(false); }
  }, e => { setMessage(geoErrorText(e),true); setLiveUi(false); }, {enableHighAccuracy:true,maximumAge:0,timeout:15000});
}

function requestCurrentPosition() {
  if (!("geolocation" in navigator)) return setMessage("Géolocalisation non disponible.",true);
  if (!sharingEnabled) { sharingToggle.checked = true; startSharing(); return; }
  setMessage("Recherche de ta position…");
  navigator.geolocation.getCurrentPosition(p => persistPosition(p,true).then(() => focusUser(false)).catch(() => setMessage("Impossible d’actualiser ta position.",true)),e => setMessage(geoErrorText(e),true),{enableHighAccuracy:true,maximumAge:0,timeout:15000});
}

async function setupRealtime() {
  if (realtimeChannel) await supabase.removeChannel(realtimeChannel);
  realtimeChannel = supabase.channel("couple-live-location").on("postgres_changes",{event:"*",schema:"public",table:"current_locations"},() => loadPositions({quiet:true})).subscribe();
}

function bindUi() {
  sharingToggle?.addEventListener("change",() => { if (sharingToggle.checked) startSharing(); else stopSharing(true).then(() => setMessage("Partage mis en pause.")); });
  $("centerOnMeBtn")?.addEventListener("click",requestCurrentPosition); $("locatePill")?.addEventListener("click",requestCurrentPosition); $("refreshLocationBtn")?.addEventListener("click",() => loadPositions().then(() => fitEveryone())); $("focusMeBtn")?.addEventListener("click",() => focusUser(false)); $("focusPartnerBtn")?.addEventListener("click",() => focusUser(true)); $("fitBothBtn")?.addEventListener("click",() => fitEveryone()); $("familyBtn")?.addEventListener("click",() => fitEveryone()); $("layersBtn")?.addEventListener("click",() => setMessage("Vue carte active.")); $("settingsBtn")?.addEventListener("click",() => setMessage(sharingEnabled ? "Partage de position activé." : "Partage de position désactivé.")); $("inboxBtn")?.addEventListener("click",() => setMessage("Messages de Nous Deux.")); $("chatBtn")?.addEventListener("click",() => { window.location.href="index.html#us"; }); $("sosBtn")?.addEventListener("click",() => setMessage("Configuration SOS à ajouter ensuite.")); $("navLocation")?.addEventListener("click",() => fitEveryone()); $("navDrive")?.addEventListener("click",() => setMessage("Module Au volant à ajouter ensuite.")); $("navSafety")?.addEventListener("click",() => setMessage("Module Sécurité à ajouter ensuite.")); $("navAccount")?.addEventListener("click",() => setMessage("Module Abonnement à ajouter ensuite."));
}
...
async function init() {
  ...
  currentUser = data.session.user; displayNames(); bindUi(); await loadPositions(); const resume = await loadSharingPreference(); await setupRealtime(); if (resume) startSharing(); setInterval(() => renderPositions(lastPositions),30000);
}

init().catch(error => { console.error("location init failed",error); setMessage("La localisation n’a pas pu démarrer.",true); });
